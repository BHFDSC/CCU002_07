...
# MAGIC %md # CCU002_07-D09a-covariates
# MAGIC  
...
# codelist_out = spark.table(path_out_codelist_ecds_outcome)
codelist_com = spark.table(path_out_codelist_comorbidity)
codelist_cev = spark.table(path_out_codelist_cev)
...
individual_censor_dates = cohort\
  ...
  .where(f.col('DOB') < f.to_date(f.lit('2020-01-01')))\
  .withColumnRenamed('DOB', 'CENSOR_DATE_START')\
  .withColumn('CENSOR_DATE_END', f.to_date(f.lit('2020-01-01')))
...
# restricted to individuals with DOB before 2020-01-01
...
_gdppr = (gdppr
          ...
          .where((f.col('RECORD_DATE') > f.to_date(f.lit('2019-01-01'))) & (f.col('RECORD_DATE') <= f.to_date(f.lit('2020-01-01'))))
          .distinct())
...
# MAGIC %md ### 7.2.1 ICD10 codelist matching
...
count_var(_hx_com_all_age_restrict, 'PERSON_ID'); print()  
...
_hx_com_all_age_restrict = (
   _hx_com_all_age_restrict
   .where(f.col('flag_age_restriction') == 0)
)
tmpt = tab(_hx_com_all_age_restrict, 'flag_age_restriction'); print()

# COMMAND ----------

_dict_codelist_copy = _hx_com_in.copy()
_dict_codelist_copy['all'] = ['', '', '']

print('--------------------------------------------------------------------------------------', flush=True)
print('summarise by name and source', flush=True)
print('--------------------------------------------------------------------------------------', flush=True)
_n_list = []
_out_summ_name = []
for i, key in enumerate(_dict_codelist_copy):
  print(i, key)
  _out = _hx_com_all_age_restrict
  if(key != 'all'): _out = _out.where(f.col('source') == key) # _dict_codematch[key]
...
for i, key in enumerate(_dict_codelist_copy):
  if(key != 'all'):
    ...
    _out = _hx_all_3.where(f.col('source') == key) # _dict_codematch[key]
    _codelist = _dict_codelist_copy[key][1]
    print(i, key, _codelist)
...
count_varlist(codelist_cvd, ['name'])
...
# merge _lsoa, _n_consultations, and _unique_bnf_chapters
...
# save name
outName = f'{proj}_out_inf_covariates'.lower()
...
for i, table in enumerate(list_tables):
  print(i, table)
  outName = f'{proj}_out_inf_codelist_match_cov{table}'.lower()
  tmp1 = globals()[table]
