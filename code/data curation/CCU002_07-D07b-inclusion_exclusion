...
# MAGIC %md # CCU002_07-D07b-inclusion_exclusion
# MAGIC  
# MAGIC **Description** This notebook creates the inclusion exclusion table for vaccination.
# MAGIC  
...
# MAGIC
# MAGIC 1.Exclusion of patients aged >18 at baseline (2021-08-06)
# MAGIC 2.Exclusion of patients aged 0-4 years old
# MAGIC 3.Exclusion of patients not in GDPPR
# MAGIC 4.Exclusion of patients who died before baseline (2021-08-06)
# MAGIC 5.Exclusion of patients with region missing or outside England
# MAGIC 6.Exclusion of patients without LSOA
# MAGIC 7.Exclusion of patients without missing, Pfizer, Pfizer child
# MAGIC 8.Exclusion of patients who failed the quality assurance
...
spark.sql(f"""REFRESH TABLE {path_out_vacc_covariates_lsoa}""")
...
cov_lsoa   = spark.table(path_out_vacc_covariates_lsoa)
...
print('---------------------------------------------------------------------------------')
# check
count_var(cov_lsoa, 'PERSON_ID'); print()
...
print('---------------------------------------------------------------------------------')
# check
count_var(vaccination, 'PERSON_ID'); print()

print('---------------------------------------------------------------------------------')
print('merged')
print('---------------------------------------------------------------------------------')
# merge in vaccination
# # add baseline_date

_merged = (
  merge(_merged, vaccination, ['PERSON_ID'], validate='1:1', keep_results=['both', 'left_only'])
  .withColumn('vaccination', f.when(f.col('_merge') == 'both', 1).otherwise(0))
  .drop('_merge')
  .withColumn('baseline_date', f.when(f.col('DOB') > '2021-08-06', f.col('DOB')).otherwise(f.lit('2021-08-06')))); print() 

# check
count_var(_merged, 'PERSON_ID'); print()

# temp save
outName = f'{proj}_tmp_inclusion_exclusion_merged_vacc'.lower()
...
# filter out adults > 18 (keep those <=18 [i.e., born on or after 2003-08-06])
# this statment will remove ~14m individuals with missing DOB from the skinny table who failed the qa
tmp1 = tmp0.where(f.col('DOB') >= '2003-08-06')
...
# MAGIC %md ## 3.2 Exclude patients with age 0-4

# COMMAND ----------

# filter out patients with Age 5 years old and over on the study start date
# Note : Someone who turn 5 on 2021-08-06 is born on 2016-08-06. Anyone born on or before that should be included.

tmp2 = tmp1.where(f.col('DOB') <= f.to_date(f.lit('2016-08-06')))

# check
tmpt = count_var(tmp2, 'PERSON_ID', ret=1, df_desc='post exclusion of patients aged < 5 at baseline (2016-08-06)', indx=2); print()
tmpc = tmpc.unionByName(tmpt)

# COMMAND ----------

# MAGIC %md ## 3.3 Exclude patients not in GDPPR

# COMMAND ----------

# filter out patients not in GDPPR
tmp3 = tmp2.where(f.col('in_gdppr') == 1)

# check
tmpt = count_var(tmp3, 'PERSON_ID', ret=1, df_desc='post exclusion of patients not in GDPPR', indx=3); print()
tmpc = tmpc.unionByName(tmpt)
...
# MAGIC %md ## 3.4 Exclude patients who died before baseline
...
tmp3 = (
  ...
  .withColumn('flag_baseline_gt_20210806', f.when(f.col('baseline_date') > f.to_date(f.lit('2021-08-06')), 1).otherwise(0)))

# check
tmpt = tab(tmp3, 'flag_DOD_lt_baseline', 'flag_baseline_gt_20210806', var2_unstyled=1); print()

# filter out and tidy
tmp4 = (
  tmp3
  .where(f.col('flag_DOD_lt_baseline') == 0)
  .drop('flag_DOD_lt_baseline', 'flag_baseline_gt_20210806'))

# check
tmpt = count_var(tmp4, 'PERSON_ID', ret=1, df_desc='post exlusion of patients who died before baseline', indx=4); print()
tmpc = tmpc.unionByName(tmpt)
...
tmp5 = tmp4.where(~f.col('region').isin(['None','Scotland','Wales']))

# check
tmpt = count_var(tmp5, 'PERSON_ID', ret=1, df_desc='post exclusion of patients with region missing or outside England', indx=5); print()

tmpc = tmpc.unionByName(tmpt)

# COMMAND ----------

tmp4.groupBy("region").count().show()

# COMMAND ----------

tmp5.groupBy("region").count().show()
...
# MAGIC %md ## 3.7 Exclude patients without Pfizer, Pfizer child vaccines

# COMMAND ----------

# Has a record of null, Pfizer, Pfizer vaccine

tmp7 = tmp6.where( (f.col('PRODUCT_1').isNull()) | (f.col('PRODUCT_1').isin(['Pfizer','Pfizer child'])))

# check
tmpt = count_var(tmp7, 'PERSON_ID', ret=1, df_desc='post exclusion of patients without Pfizer, Pfizer child vaccines', indx=7); print()
tmpc = tmpc.unionByName(tmpt)

# COMMAND ----------

# MAGIC %md ## 3.8 Exclude patients who failed the quality assurance
...
# check
tmpt = tab(tmp7, '_rule_total', 'flag_rule_total_gt_0', var2_unstyled=1); print()
tmpt = tab(tmp7, 'flag_rule_total_gt_0', 'flag_baseline_gt_20210806', var2_unstyled=1); print()

# filter out and tidy
tmp8 = (
  tmp7
  .where(f.col('flag_rule_total_gt_0') == 0)
  .drop('flag_rule_total_gt_0', 'flag_baseline_gt_20210806'))

# check
tmpt = count_var(tmp8, 'PERSON_ID', ret=1, df_desc='post exclusion of patients who failed the quality assurance', indx=8); print()
tmpc = tmpc.unionByName(tmpt)
...
tmpf = tmp8.select('PERSON_ID', 'DOB', 'SEX', 'ETHNIC', 'ETHNIC_DESC', 'ETHNIC_CAT', 'DOD', 'LSOA', 'lsoa_name',
                   'region', 'IMD_2019_DECILES', 'date_1', 'PRODUCT_1', 'date_2', 'PRODUCT_2', 'vaccination', 'baseline_date')
...
# save name
outName = f'{proj}_out_inc_exc_vacc_cohort'.lower()
...
# save name
outName = f'{proj}_out_vacc_flow_inc_exc'.lower()
