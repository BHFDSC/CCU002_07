...
dataset = readRDS('cox_infec_clean_ecds.rds')
start_date = as.IDate("2020-01-01")
end_date = as.IDate("2022-03-31")
...
weeks_since_covid<-c(rep('1d',1), rep('2d-1w',6),rep('2-4w',21),rep('5-26w',154), rep('27w-365d',183)
                     ,rep(paste0('366d-',max(fu),'d'),max(fu)-366+1))

hr_table = fread(paste0('~/collab/CCU002_07/infection/cox_wen/compiled_results/suppressed_compiled_HR_results_out_',i,'_date_covid_TO_RELEASE.csv'))
hr_table = hr_table[time_points=='normal'&model=='mdl_max_adj_region'&substr(term,1,4)=='days',.(hr,conf_low,conf_high)]
times =c(1,6,21,154,183,max(fu)-366+1)
HR = c()
HR_LB = c()
HR_UB = c()
for (j in 1:6)
...
p_line = plot_graph(num_subgroups = 0,
                    ...
                    graph_name = i,
                    color_value = 'black')
save_plot('~/collab/CCU002_07/infection/abs_risk_plot',p_line,paste0(i,'_inf'),700,10,10)
}

# Saving a data frame with readr's write_csv
write_csv(life_table_ate, "life_table_ate_inf.csv")
write_csv(life_table_carditis, "life_table_carditis_inf.csv")
write_csv(life_table_inflammatory, "life_table_inflammatory_inf.csv")
write_csv(life_table_vte, "life_table_vte_inf.csv")
write_csv(life_table_thrombocytopenia, "life_table_thrombocytopenia_inf.csv")



# All data together for both cohorts
setwd("/db-mnt/databricks/rstudio_collab/CCU002_07/abs_risk")
# Paths to the files
file_paths <- list.files(path = ".", pattern = "life_table_.*\\.csv", full.names = TRUE)

# Read and label data
read_and_label <- function(filepath) {
  data <- read_csv(filepath)
  # Extracting conditions and cohort type from the filename
  parts <- strsplit(x = basename(filepath), split = "_|\\.")[[1]]
  condition <- parts[3]
  cohort_type <- parts[4]
  data <- mutate(data, Condition = condition, Cohort = cohort_type)
  return(data)
}

# Read all data files and bind them into one dataframe
all_data <- map_df(file_paths, read_and_label)

# Create a named vector for mapping abbreviations to full names
condition_mapping <- c(ate = "Arterial Thrombotic Events",
                       vte = "Venous Thrombotic Events",
                       thrombocytopenia = "Thrombocytopenia",
                       carditis = "Myocarditis/Pericarditis",
                       inflammatory = "Inflammatory Conditions")

# Update the dataset to replace abbreviations with full names
all_data <- all_data %>%
  mutate(Condition = factor(Condition, levels = names(condition_mapping), labels = condition_mapping))

# Update the dataset to replace cohort abbreviations with full names
all_data <- all_data %>%
  mutate(Cohort = recode(Cohort,
                         'inf' = 'Infection',
                         'vax' = 'Vaccination'))

# Ensure the levels of the factor are updated if Cohort is a factor
all_data$Cohort <- factor(all_data$Cohort)

# Plotting the data with full condition names
ggplot_plot <- ggplot(all_data, aes(x = x_days_since_covid, y = AER_percent, color = Cohort)) +
  geom_line() +
  facet_wrap(~Condition, scales = "fixed") +  # Facets display full condition names
  labs(#title = "Absolute Excess Risk per Day Across Conditions",
       x = "Time (days)",
       y = "Difference in cumulative absolute risk (%)",
       color = "Cohort") +
  scale_color_manual(values = c("Infection" = "red", "Vaccination" = "blue")) +
  theme_minimal()
...
ggsave("AER_inf_vax_alldays.png", plot = ggplot_plot, width = 12, height = 8)


# Assuming 'all_data' is your full dataset
# Filter the data for the first 180 days and update cohort names
filtered_data <- all_data %>%
  filter(x_days_since_covid <= 180)

# Ensure the levels of the factor are updated if Cohort is a factor
filtered_data$Cohort <- factor(filtered_data$Cohort)

# Plotting the data with full condition names
ggplot_plot <- ggplot(filtered_data, aes(x = x_days_since_covid, y = AER_percent, color = Cohort)) +
  geom_line() +
  facet_wrap(~Condition, scales = "fixed") +  # Facets display full condition names
  labs(#title = "Absolute Excess Risk per Day Across Conditions",
    x = "Time (days)",
...
# Save the plot to a file
ggsave("AER_inf_vax_180days.png", plot = ggplot_plot, width = 12, height = 8)

# Filter data for day 180
day_180_data <- all_data %>%
  filter(x_days_since_covid == 180) %>%
  select(Condition, Cohort, AER_percent, AER, AER_LB, AER_percent_LB, AER_UB, AER_percent_UB)
