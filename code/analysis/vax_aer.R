...
to_calculate_futime<-function(cohort_start, vax_date, death_date, disease_event_date, max_follow_up_time){
  futime<-ifelse(is.na(vax_date) & is.na(death_date) & is.na(disease_event_date), max_follow_up_time, 
                 ifelse(is.na(vax_date)& !is.na(disease_event_date), as.numeric(disease_event_date - cohort_start)+1,
                        ifelse(is.na(vax_date) & !is.na(death_date), as.numeric(death_date-cohort_start)+1,
                               ifelse(!is.na(vax_date) & is.na(disease_event_date), as.numeric(vax_date-cohort_start),
                                      ifelse(!is.na(vax_date) & disease_event_date<vax_date, as.numeric(disease_event_date-cohort_start)+1,
                                             ifelse(!is.na(vax_date) & disease_event_date>vax_date, as.numeric(vax_date-cohort_start), 
                                                    ifelse(!is.na(vax_date) & disease_event_date==vax_date, as.numeric(vax_date-cohort_start),'to check')))))))
...
average_daily_incidence<-function(dataset, disease_event_date, vax_date,futime,max_follow_up_time){
  ...
  disease_status<-ifelse(is.na(disease_event_date),0,
                         ifelse(!is.na(disease_event_date) & is.na(vax_date),1,
                                ifelse(!is.na(disease_event_date) & disease_event_date>=vax_date,0,1))) 
  ...
  exposed_individual<-sum(!is.na(dataset$vax_date_covid_1))  #number of individuals (exposed) with a COVID-19 diagnosis during cohort follow-up
...
incidence_rate<-function(disease_event_date,vax_date,futime){
  
  #calculates the nominator
  total_event<-sum(!is.na(disease_event_date)) 
  event_exposed<-sum(disease_event_date>=vax_date,na.rm = T)
...
AER_calculation<-function(dataframe_name,event_name,subgroup_name,q,HR,HR_LB,HR_UB,covid_vax_population,total_num_rows_for_life_table,weeks_since_vax){
  event<-rep(event_name,total_num_rows_for_life_table) 
  subgroup<-rep(subgroup_name,total_num_rows_for_life_table)
  x_days_since_vax<-c(1:total_num_rows_for_life_table)
  weeks_since_vax<-weeks_since_vax
  q_estimated_average_incidence_rate<-rep(q, total_num_rows_for_life_table)
  life_table<-data.frame(event,subgroup,x_days_since_vax,weeks_since_vax,q_estimated_average_incidence_rate,HR,HR_LB,HR_UB)
  ...
  excess_mean<-covid_vax_population*life_table[total_num_rows_for_life_table,20]
  excess_LB<-covid_vax_population*life_table[total_num_rows_for_life_table,22]
  excess_UB<-covid_vax_population*life_table[total_num_rows_for_life_table,24]
  return_list<-list(excess_events_mean=excess_mean, excess_events_LB=excess_LB, excess_events_UB=excess_UB)
  return(return_list) #this function returns changes in AER, total number of excess disease events attributable to COVID-19 vaccination and the 95% confidence intervals for excess events
}
...
plot_graph<-function(num_subgroups, life_table1,lifetable2,lifetable3,lifetable4,lifetable5, graph_title,graph_name,levels,legend_title,color_value,linetype){
  ...
  if(num_subgroups==0){#if there is no subgroup, i.e. plotting for all and not split by sex for example, put num_subgroups=0 in the function argument.
    ...
    life_table1$x_days_since_vax_converted<-life_table1$x_days_since_vax/7
    
    p_line<-ggplot(life_table1,
                   aes(x=x_days_since_vax_converted,
                       y=AER_percent,
                       lty = 'All')) +
      ...
      labs(x='Weeks since COVID-19 vaccination',y='Difference in cumulative absolute risk (%)',
           title = graph_title)+
  ...
  combined$x_days_since_vax_converted<-combined$x_days_since_vax/7
...
# ==============================================================================================
setwd('~/collab/CCU002_07/vaccination/data')
dataset = readRDS('cox_vax_ecds_clean.rds')
start_date=as.IDate("2021-08-06")
end_date = as.IDate("2022-12-31")

dataset = data.table(dataset)
dataset[,'baseline_date':=as.IDate(baseline_date)]
dataset[,'vax_date_covid_1':=as.IDate(vax_date_covid_1)]
...
outname = list(ate='Arterial thrombotic events', vte='Venous thrombotic events',thrombocytopenia='Thrombocytopenia',carditis='Myocarditis/Pericarditis',inflammatory='Inflammatory conditions'
               ,fracture='Limb fracture (control)'
)

fu = as.numeric(dataset$fu_end_date2- dataset$baseline_date+1)

for (i in c('ate','vte','carditis','thrombocytopenia','inflammatory','fracture'))
{futime =to_calculate_futime(dataset$baseline_date,dataset$vax_date_covid_1,dataset$death_date,dataset[,get(paste0('out_',i,'_date'))],fu)

# futime3 = dataset[,as.numeric(pmin(vax_date_covid_1,out_ate_date,fu_end_date2,na.rm=TRUE)-baseline_date+1)]
avg = average_daily_incidence(dataset,dataset[,get(paste0('out_',i,'_date'))],dataset$vax_date_covid_1,futime,max(fu))
rate = incidence_rate(dataset[,get(paste0('out_',i,'_date'))],dataset$vax_date_covid_1,futime)

weeks_since_vax<-c(rep('1d-1w',7),rep('1-4w',21),rep('5-26w',154)
                     ,rep(paste0('183d-',max(fu),'d'),max(fu)-183+1))

hr_table = fread(paste0('~/collab/CCU002_07/vaccination/cox/19_01_2024_ecds/compiled_results/suppressed_compiled_HR_results_out_',i,'_date_vaccine_TO_RELEASE.csv'))
hr_table = hr_table[time_points=='normal'&model=='mdl_max_adj_region'&substr(term,1,4)=='days',.(hr,conf_low,conf_high)]
times =c(7,21,154,max(fu)-183+1)
HR = c()
HR_LB = c()
HR_UB = c()
for (j in 1:4)
...
AER_calculation(dataframe_name = paste0('life_table_',i), 
                ...
                subgroup_name = 'All',
                weeks_since_covid = weeks_since_vax,
                q=avg['average_daily_incidence'],
                HR=HR,
                HR_LB=HR_LB,
                HR_UB=HR_UB,
                covid_vax_population = avg['number_of_exposed_individuals'], 
...
p_line = plot_graph(num_subgroups = 0,
                    ...
                    graph_name = i,
                    color_value = 'black')
save_plot('~/collab/CCU002_07/vaccination/output',p_line,i,700,10,10)
}

library(readr)

# Saving a data frame with readr's write_csv
write_csv(life_table_ate, "life_table_ate_vax.csv")
write_csv(life_table_carditis, "life_table_carditis_vax.csv")
write_csv(life_table_inflammatory, "life_table_inflammatory_vax.csv")
write_csv(life_table_vte, "life_table_vte_vax.csv")
write_csv(life_table_thrombocytopenia, "life_table_thrombocytopenia_vax.csv")
